...
//  }
    ...
    if(profile_idc >= 100) //high profile
	{
        ue_v(stream, &temp);
        if(temp == 3)
            BitstreamRead1Bit(stream, &temp);  //residual_color_transform_flag
        ue_v(stream, &temp);  //bit_depth_luma_minus8
        ue_v(stream, &temp);  //bit_depth_chroma_minus8
        BitstreamRead1Bit(stream, &temp); //transform_bypass
        BitstreamRead1Bit(stream, &temp); //scaling_matrices_flag
        if(temp)
    	{
    		return AVCDEC_FAIL;
    	}
...
AVCDec_Status DecodePPS(AVCDecObject *decvid, AVCCommonObj *video, AVCDecBitstream *stream)
{
    ...
    {
        //status = AVCDEC_FAIL;	/////// RK28 support this furture!
        //goto clean_up;
    }
